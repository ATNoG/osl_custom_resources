...
def on_create_network_slice_ue(spec, meta, logger, **kwargs):
    nslice_cr_handler.process_nslice_ue_event("ADD", spec, meta)

@kopf.on.update(Config.cr_group, Config.cr_version, Config.cr_plural)
def on_update_network_slice_ue(spec, old, new, diff, meta, logger, **kwargs):
    nslice_cr_handler.process_nslice_ue_event("UPDATE", spec, meta)
